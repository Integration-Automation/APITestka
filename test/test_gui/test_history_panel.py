...
def test_push_and_latest():
    ...
    model.push(HistoryEntry(method="GET", url="https://x.invalid", status=200))
...
def test_buffer_respects_limit():
    ...
    for index in range(5):
        model.push(HistoryEntry(method="GET", url=f"https://x.invalid/{index}"))
    assert len(model.all()) == 2


def test_clear_empties_buffer():
    model = HistoryPanelModel()
    model.push(HistoryEntry(method="GET", url="https://x.invalid"))
    model.clear()
    assert model.all() == []
