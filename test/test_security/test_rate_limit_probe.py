...
def test_triggers_when_server_returns_429():
    ...
    result = probe_rate_limit("http://x.invalid", burst=10, transport=transport)
...
def test_no_trigger_returns_false():
    ...
    result = probe_rate_limit("http://x.invalid", burst=3, transport=transport)
    assert result.triggered is False
    assert result.triggered_at_attempt is None


def test_zero_burst_short_circuits():
    result = probe_rate_limit("http://x.invalid", burst=0)
    assert result.triggered is False
