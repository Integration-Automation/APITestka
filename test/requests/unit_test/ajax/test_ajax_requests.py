...
if __name__ == "__main__":
    ...
    test_response_post = test_api_method_requests("session_post", login_url, headers=login_headers, data=payload)
    if test_response_post is not None:
        ...
        test_response_get = test_api_method_requests("session_get", welcome_url, headers=login_headers)
        print(test_response_get.get("response_data").get("status_code"))
        print(test_response_get.get("response_data").get("text"))

    try:
        test_response_get = test_api_method_requests("dwadwadadw", "dwadawdwdadw", headers={"aa": "ttawtwafaw"})
    except Exception as error:
        print(repr(error), file=sys.stderr)
    try:
        new_test_response_get = test_api_method_requests("get", "dwadawdwdadw", headers={"aa": "ttawtwafaw"})
